...
def logare():
    ...
    if form.validate_on_submit():
        ...
        flash("Nu aveti dreptul de a folosi aceasta sectiune. Creati-va in cont pentru a putea intra !")
...
def cont_nou():
    ...
    if form.validate_on_submit():
        ...
        hashed_parola = generate_password_hash(form.parola.data, method='sha256')
        utilizator_nou = Login(nume=form.nume.data,
                               prenume=form.prenume.data,
                               utilizator=form.utilizator.data,
                               email=form.email.data,
                               data_adaugare=datetime.now(),
                               data_modificare=datetime.now(),
                               parola=hashed_parola)
...
def cursant():
    ...
    an_curent = datetime.now()
    if form.validate_on_submit:
        cursant_nou = Clienti(nume=form.nume.data,
                              prenume=form.prenume.data,
                              categorie_scolarizare=form.categorie_scolarizare.data,
